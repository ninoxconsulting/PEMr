...
fuzz_matrix <- read.csv(file.path(out_dir, "fuzzy_matrix.csv")) |> dplyr::select(target, Pred, fVal)
covars <- read.csv(fs::path(out_dir, "reduced_covariate_list.csv")) |> dplyr::pull()
nf_f_filter <- terra::rast(fs::path(PEMprepr::read_fid()$dir_3010_inputs$path_abs, "forest_nonforest_final.tif"))
