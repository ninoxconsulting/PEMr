...
```{r}
#| eval: false
## Generate the cost layer - this currently uses the original cost layer script; 

sampleplan_dir <- PEMprepr::read_fid()$dir_201010_inputs$path_rel
vec_dir = PEMprepr::read_fid()$dir_1010_vector$path_rel
out_dir = PEMprepr::read_fid()$dir_201010_inputs$path_rel
heli = FALSE


acost <- prep_cost_layer(vec_dir, sampleplan_dir, 
                         out_dir, 
                         heli = FALSE, 
                         write_output = TRUE, 
                         overwrite = FALSE)
